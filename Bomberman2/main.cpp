...
void explodirBomba(int bx, int by, int raio, Jogador& dono, bool destruirFrageis = true) {
    ...
    // Copia o mapa ANTES de destruir qualquer parede.
    // Assim a explosao nunca atravessa uma parede fragil que foi removida nesse mesmo estouro.
    ...
    // Caso especial do fantasma: se a bomba foi colocada em cima de uma parede fragil,
    // essa parede e destruida, mas a explosao NAO atravessa essa parede.
    // Assim evita o bug de destruir paredes frageis "por tras" dela.
    if (mapaAntes[bx][by] == CELULA_FRAGIL) {
        destruirParedeFragil(bx, by, dono, destruirFrageis);
        return;
    }
    ...
    for (int k = 0; k < 4; k++) {
        for (int r = 1; r <= raio; r++) {
            ...
            if (mapaAntes[nx][ny] == CELULA_FRAGIL) {
                destruirParedeFragil(nx, ny, dono, destruirFrageis);
                break; // regra classica: para na primeira parede fragil daquela direcao
            }
        }
    }
}
