...
void wavWrite4(FILE* f, unsigned int v) {
    fputc(v & 0xFF, f);
    fputc((v >> 8) & 0xFF, f);
    fputc((v >> 16) & 0xFF, f);
    fputc((v >> 24) & 0xFF, f);
...
void wavWrite2(FILE* f, unsigned short v) {
    fputc(v & 0xFF, f);
    fputc((v >> 8) & 0xFF, f);
}

void gerarWav(const char* arquivo) {
    ...
    unsigned char* samples = new unsigned char[totalSamples];
    int pos = 0;
...
void iniciarMusicaGameplay() {
    PlaySound(TEXT("bomberman_music.wav"),
              NULL,
...
void inicializarInimigos(int fase, int dificuldade) {
    ...
    if (fase == 3) {
        ...
        mapa[boss.x][boss.y] = CELULA_VAZIO;
...
void moverBot(Jogador& j, int dono) {
    ...
    // PRIORIDADE 1: Fugir se estiver em perigo
    bool emPerigo = posicaoPerigosa(j.x, j.y);
    if (emPerigo || botFugindo) {
        tentarFugir(j);
        if (!posicaoPerigosa(j.x, j.y) &&
            distanciaInimigoMaisProximo(j.x, j.y) > 2 &&
            !aindaTemBomba) {
            botFugindo = false;
        }
        return;
...
void explodirBomba(int bx, int by, int raio, Jogador& dono, bool destruirFrageis = true) {
    ...
    if (mapa[bx][by] == CELULA_FRAGIL && destruirFrageis) {
        mapa[bx][by] = CELULA_VAZIO;
        dono.caixasDestruidas++;

        if (rand() % 100 < 35) {
            bool jaExiste[7] = {};

            for (int _ie = 0; _ie < itens.tamanho(); _ie++) { Item& itemExistente = itens[_ie];
...
void aplicarDanoExplosao() {
    ...
    for (int _ii = 0; _ii < inimigos.tamanho(); _ii++) { Inimigo& in = inimigos[_ii];
        if (in.vivo && areaExplosao[in.x][in.y]) {
            in.vivo = false;
...
void moverInimigos() {
    ...
    if (frameInimigo < intervalo) return;
    ...
    for (int _ii = 0; _ii < inimigos.tamanho(); _ii++) { Inimigo& in = inimigos[_ii];
        ...
        if (in.inteligente) chancePerseguir = 90;
